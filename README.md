...
- **Building & releasing:** see **[RELEASE.md](RELEASE.md)** for the build → sign → tag → publish process and the review gates each release passes. Packages must be built **on an OPNsense box** — GitHub Actions has no OPNsense/FreeBSD runner, and the dependency name differs from stock FreeBSD (OPNsense 26.x uses `py313-scapy`).
