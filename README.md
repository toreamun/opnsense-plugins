...
- **HA config sync** *(optional)*: replicate the keeper config to the peer (System ‣ High Availability ‣ Settings), so you configure once on the master. Safe: the config is node-agnostic. Note that OPNsense's sync only copies the settings: the peer's keeper keeps running its previous settings until it is restarted, so after a keeper change press *Synchronize and reconfigure all* under System ‣ High Availability ‣ Status (or restart the keeper on the peer).
